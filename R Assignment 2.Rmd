...
set.seed(1000)
num_sim_ci <- 1000                  
phat_sim_ci <- numeric(num_sim) 

for (i in 1:num_sim_ci){
  cur_samp_ci <- sample(c(0,1), size = num_sim_ci, replace = TRUE
                     , prob = c(1-p_hat,p_hat))
  phat_sim_ci[i] <- mean(cur_samp_ci)
}
```
...
```{r}
round(quantile(phat_sim_ci, probs <- c(0.05,0.95)),2)
```
**Step 3 : Conclusion**

 We are 90% confident that the true proportion of soy seeds sprouted in 2023 is in the interval **(0.91,0.93)**.
